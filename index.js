...
function calculateCircleArea(radius) {
  ...
  // Повертаємо обчислену площу кола.
   if (typeof radius !== 'number') {
    console.log('Помилка: Радіус повинен бути числом.');
    return null;
  }
...
function findMinMax(numbers) {
  ...
  if(!Array.isArray(numbers)){
    console.log('Помилка: Параметр повинен бути масивом.');
    return null;
...
function calculateHypotenuse(a, b) {
 ...
 if (typeof a !== 'number' || typeof b !== 'number') {
        console.log('Помилка: два параметри має бути числами.');
        return null;
    }
...
function roundObjectValues(obj) {
  ...
  const roundedEntries = Object.entries(obj).map(([key, value]) => {
...
function calculateVolumeCylinder(radius, height) {
  ...
  // Повертаємо обчислений об'єм.
   if (typeof radius !== 'number' || typeof height !== 'number') {
    console.log('Помилка: обидва аргументи повинні бути числами.');
    return null;
  }
...
function estimateInvestment(principal, interestRate, years) {
  ...
  // Повертаємо розраховану суму інвестицій.
   if (typeof principal !== 'number' || typeof interestRate !== 'number' || typeof years !== 'number') {
    ...
    const value = principal * Math.pow(1 + interestRate, years);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (!Array.isArray(products)) {
    console.log("Помилка: products має бути масивом.");
    return null;
  }

  if (typeof maxPrice !== "number") {
    console.log("Помилка: maxPrice має бути числом.");
    return null;
  }
